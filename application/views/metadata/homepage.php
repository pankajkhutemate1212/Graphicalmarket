					...
					<?php if(!empty($newsdata))
						{ 
							foreach($newsdata as $row)
							{
								...
								<article class="herald-lay-b post-99 post type-post status-publish format-standard has-post-thumbnail hentry category-finance-news category-general-news category-technology-news tag-softbank tag-softbank-investment tag-softbank-latin-america tag-vtex">
<div class="row">
			...
			<div class="herald-post-thumbnail herald-format-icon-middle">
				<a href="<?php echo base_url(''); echo $row->url;echo '/';?>" title="<?php echo $row->name; ?>">
                              ...
                              <?php if (file_exists($_SERVER['DOCUMENT_ROOT'].'/'.$filename)) {   ?>
											<img  width="300" height="200" src="<?php echo base_url($filename);?>" class="attachment-herald-lay-b1 size-herald-lay-b1 wp-post-image" alt="<?php echo str_replace("&","&amp;",$row->name); ?>" title="">
										<?php } else {?>
											<img width="300" height="200" src="<?php echo base_url('assets/imgs/news-'.$img.'.jpg');?>" class="attachment-herald-lay-b1 size-herald-lay-b1 wp-post-image" alt="<?php echo str_replace("&","&amp;",$row->name); ?>" title="">
										<?php } ?>
									</a>
			</div>
		</div>
	...
	<div class="col-lg-8 col-md-8 col-sm-8">
		<div class="entry-header">
							<span class="meta-category"><a href="<?php echo base_url('category/'.$row->type);echo '/';?>"class="herald-cat-3"><?php echo str_replace("-"," ",$row->type); ?></a></span>
			
			<h2 class="entry-title h3"><a href="<?php echo base_url('');echo $row->url;echo '/';?>" title="<?php echo str_replace("&","&amp;",$row->name); ?>"><?php echo str_replace("&","&amp;",$row->name); ?></a></h2>
				...
				<p><?php echo substr(strip_tags($row->full_desc),0, 200);?>....</p>
			</div>
